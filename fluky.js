;(function(global) {
	...
	let Fluky = (...args) => {
		...
		// If so, change to 20 and shighest message to the console. 
	...
	class FlukyInit {
		...
		constructor(lowest = 1, highest = 100, fixed = 0) {
			...
			// Hold the return value and number of attempts for each 
			// invocation of the new() method. 
			this.value = '';
			this.attempts = 0;
		}
		new() {
			...
			// value of the 'highest' parameter.
			var randomNumber = Math.random() * this.highest;

			// Add 1 to the 'attempts' property each time.
			this.attempts += 1;
		...
		checker(num) {
			...
			// is greater than the 'lowest' value provided by the user
			...
			if (num < this.lowest) {
				...
				// Once the  generated number is greater than the 'lowest'
				// value, use the toFixed() method to set the number of
				// decimal places, passing in the user's 'fixed' argument provided.
